...
from mcpturbo_agents import ExternalAgent, AgentCapability
...
class OpenAIAgent(ExternalAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model = model
        self.headers.update({"OpenAI-Beta": "assistants=v1"})
    ...
    def _parse_openai_response(
        self, response: Dict[str, Any], request
    ) -> Dict[str, Any]:
...
class ClaudeAgent(ExternalAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model = model
        self.headers.update({"anthropic-version": "2023-06-01", "x-api-key": api_key})
    ...
    def _parse_claude_response(
        ...
        if action == "reasoning":
            parts = text.split("\n\n")
            result.update(
                {"reasoning": text, "conclusion": parts[-1] if len(parts) > 1 else text}
            )
        elif action == "analysis":
            result.update({"analysis": text, "insights": self._extract_insights(text)})
...
class DeepSeekAgent(ExternalAgent):
    ...
    def build_payload(self, request) -> Dict[str, Any]:
        ...
        if action == "fast_coding":
            language = data.get("language", "python")
            prompt = (
                f"Write efficient {language} code for: {prompt}\n"
    ...
    def _parse_deepseek_response(
        ...
        if action == "fast_coding":
            ...
            result.update({"code": code[0] if code else content, "optimized": True})
        elif action == "code_optimization":
            code = self._extract_code_blocks(content)
            result.update(
                {
...
def create_claude_agent(
    api_key: str, model: str = "claude-3-sonnet-20240229", **kwargs
) -> ClaudeAgent:
